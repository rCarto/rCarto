...
function(fixBrks,listBrks,pt,var,nclass,
  ...
  if(fixBrks==TRUE){
    ...
    distr<-classInt::classIntervals(pt[is.na(pt[,var])==FALSE,var],nclass,style="fixed",fixedBreaks=listBrks)$brks
  }else{distr<-classInt::classIntervals(pt[is.na(pt[,var])==FALSE,var],nclass,style=style)$brks}
